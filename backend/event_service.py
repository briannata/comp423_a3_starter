...
class EventService:
    ...
    def create(self, event: Event) -> Event:
        ...
        If the event's ID already exists in the table, the existing entry is updated.
        ...
        if self._session.get(EventEntity, event.id):
            ...
            self._session.execute(
                update(EventEntity)
                .where(EventEntity.id == event.id)
                .values(
                    id=event_entity.id, 
                    name=event_entity.name, 
                    logo=event_entity.logo, 
                    short_description=event_entity.short_description, 
                    long_description=event_entity.long_description, 
                    website=event_entity.website, 
                    email=event_entity.email, 
                    instagram=event_entity.instagram, 
                    linked_in=event_entity.linked_in, 
                    youtube=event_entity.youtube, 
                    heel_life=event_entity.heel_life
            ))
